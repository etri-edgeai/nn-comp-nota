...
if args.arch == 'vgg_16_bn':
    ...
    for i, cov_id in enumerate(relucfg):
        ...
        np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank' + str(i + 1) + '.npy',
    ...
    if not os.path.isdir('rank_conv/' + args.arch + '_limit%d' % (args.limit)):
        os.mkdir('rank_conv/' + args.arch + '_limit%d' % (args.limit))
    np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (1) + '.npy',
            feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for i in range(3):
        ...
        for j in range(9):
            ...
            test()
            handler.remove()
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (cnt + 1) + '.npy',
                    feature_result.numpy())
            cnt += 1
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
    ...
    for i in range(3):
        ...
        for j in range(12):
            ...
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (
                        13 * i + j + 1) + '.npy', feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
        ...
        if i < 2:
            ...
            np.save(
                'rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (13 * (i + 1)) + '.npy',
                feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)  # '''
    ...
    test()
    handler.remove()
    np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (39) + '.npy',
            feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for idx, cov in enumerate(cov_list):
        ...
        if idx > 0:
            for idx1, tp in enumerate(tp_list):
                if idx1 == 3:
                    np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d_' % (
                                idx + 1) + tp + '.npy',
            ...
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (idx + 1) + '.npy',
                    feature_result.numpy())
        feature_result = torch.tensor(0.)
        total = torch.tensor(0.)
    ...
    for i in range(4):
        ...
        for j in range(net.num_blocks[i]):
            ...
            if j == 0:
                np.save(
                    'rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv_hrank%d' % (cnt + 1) + '.npy',
